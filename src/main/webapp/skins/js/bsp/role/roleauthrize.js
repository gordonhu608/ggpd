...
function initMenuTree(typeId,typeName) {
	...
	var treeSetting = {
			async : {
				enable: true,
				url: context + "/service/bsp/role/getRoleMenuAuthorizeTreeData",
...
function initFunctionTree() {
	...
	var treeSetting = {
			async : {
				enable: true,
				url: context + "/service/bsp/role/getRoleNMenuAuthorizeTreeData",
...
function save(){
	...
	$.ajax({
		url:context+"/service/bsp/role/saveRoleFunctionPermit",
		data:data,
		type:"POST",
		success:function(){
			dialog.close(true);
