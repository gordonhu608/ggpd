...
function initMenuTypeSelector() {
	$.ajax({
		url: context + "/service/bsp/rolenoorg/getAllMenuTypes",
		dataType: "json",
		async: false,
		success: function(data) {
			for(var i = 0 ; i < data.length ; i++) {
				var opt = "<option value="+data[i].menuTypeId+">"+data[i].menuTypeName+"</option>";
...
function initMenuTree(typeId,typeName) {
	...
	var treeSetting = {
			async : {
				enable: true,
				url: context + "/service/bsp/rolenoorg/getRoleMenuAuthorizeTreeData",
				autoParam: [ "roleId","type", "menuTypeId", "menuStruId", "menuId","isLeaf" ],
...
function initFunctionTree() {
	var msg = L.getLocaleMessage("bsp.role.050","功能资源树");
	// 用于存储树的节点
	var treeNodes = [];
	...
	var treeSetting = {
			async : {
				enable: true,
				url: context + "/service/bsp/rolenoorg/getRoleNMenuAuthorizeTreeData",
				autoParam: [ "roleId","type", "menuId","isLeaf" ],
...
function save(){
	...
	var data ={
			...
			"moduleCodeCancelingList":returnVal.moduleCodeCancelingArray.toString(),
			"functionCodeCancelingList":returnVal.functionCodeCancelingArray.toString(),
			"operationCodeCancelingList":returnVal.operationCodeCancelingArray.toString(),
			"userDefindCodeCancelingList":returnVal.userDefinedCancelingArray.toString(),
	}
	$.ajax({
		url:context+"/service/bsp/rolenoorg/saveRoleFunctionPermit",
		data:data,
		type:"POST",
		success:function(){
			dialog.close(true);
...
function getMenuTreeResult(treeObj){
	...
	for(index in nodes){
		...
		if(nodes[index].checked==true&&nodes[index].getCheckStatus().half==false){
			if(type=="app"){
