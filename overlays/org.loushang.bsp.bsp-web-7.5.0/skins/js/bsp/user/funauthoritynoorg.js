...
function initFunctionTable() {
	...
	var url = context+"/service/bsp/usernoorg/functionData";
...
function queryUserForm(){
	...
	var url = context+"/service/bsp/usernoorg/functionData";
	var param={"roleName":roleName,"userId":$("#userId").val(),"isSelected":isSelected,"roleType":roleType};
...
$(document).on("click",".add",function() {
	...
	var msg2 = L.getLocaleMessage("bsp.user.085","角色分配成功!");
	var msg3 = L.getLocaleMessage("bsp.user.086","角色移除成功!");
	...
	if(userId == "" || userId == null || userId ==undefined){
		...
		if(isSelected=="no"){
			$.ajax({
	            url:context+"/service/bsp/usernoorg/addAutority",
		...
		if(isSelected=="yes"){
			$.ajax({
	            url:context+"/service/bsp/usernoorg/deleteAutority",
