$(function(){
	//初始化左侧表格数据
	var allUserUrl = context+"/service/bsp/rolenoorg/queryUsers";
	initUser(allUserUrl);
	//初始化右侧表格
	var roleUserUrl = context+"/service/bsp/rolenoorg/getUsersByRoleId";
	initRoleUser(roleUserUrl);
	//初始化弹窗对象
	var dialog = parent.dialog.get(window);
	//初始化岗位下拉列表
	$.ajax({
		url:context+"/service/bsp/rolenoorg/queryAllPositionType",
		dataType:"json",
...
function addRoleForUsers(userId){
	...
	$.ajax({
		url:context+"/service/bsp/rolenoorg/addRoleForUsers",
		type:"POST",
...
function delRoleForUsers(userId){
	...
	$.ajax({
		url:context+"/service/bsp/rolenoorg/delRoleForUsers",
		type:"POST",
		data:{"roleId":roleId,"userId":userId},
		success:function(){
			grid.reload();
...
function queryUserByPosition(positionCode){
	...
	if(""!=positionCode){
		url=context+"/service/bsp/rolenoorg/queryUserByPosition";
	}else{
		url = context+"/service/bsp/rolenoorg/queryUsers";
	}
