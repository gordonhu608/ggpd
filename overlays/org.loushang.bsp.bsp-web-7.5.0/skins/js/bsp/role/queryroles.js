$(function(){
	var url = context + "/service/bsp/role/queryAllRoles";
	init(url);
	$("#add").click(function(){
		window.location.href=context+"/service/bsp/role/addRolePage"
	});
...
function queryRole(struId,roleName){
	var url = context+"/service/bsp/role/queryAllRoles?roleName="+roleName+"&struId="+struId;
...
function renderstatus(data,type,full){
	var msg = L.getLocaleMessage("bsp.role.018","全局");
	if (data == "1") {
		data = msg;
	} else {
		data = full.organName;
