...
(
  ...
  (  SELECT A.ORGAN_ID,B.ORGAN_ID AS UPPER_ORGAN_ID
	 ...
	 WHERE CHARINDEX(B.STRU_PATH,A.STRU_PATH)>0 AND A.IN_USE='1' AND A.STRU_TYPE='00'
  ) C ON D.ORGAN_ID=C.ORGAN_ID 
  WHERE SUBSTRING(D.ORGAN_TYPE,1,1)='8'
...
-- 查询范围节点下的单位、部门和员工
   ...
   WHERE EXISTS (SELECT 1
           ...
           WHERE charindex(T_SCOPE.STRU_PATH, S2.STRU_PATH) > 0)
     AND S2.ORGAN_ID IS NOT NULL
	AND UD2.CODE = S3.ORGAN_ID AND UD2.STRU_TYPE = S3.STRU_TYPE
	AND charindex(S3.STRU_PATH, S2.STRU_PATH) > 0
UNION
-- 范围节点下的岗位
