...
--20111122 工作流流程统计分析需要查询员工及其对应上级组织机构的组织编号和组织名称、组织类型
...
(
  ...
  (  SELECT A.ORGAN_ID,B.ORGAN_ID AS UPPER_ORGAN_ID
	 ...
	 WHERE LOCATE(B.STRU_PATH,A.STRU_PATH)>0 AND A.IN_USE='1' AND A.STRU_TYPE='00'
  ) C ON D.ORGAN_ID=C.ORGAN_ID WHERE SUBSTR(D.ORGAN_TYPE,1,1)='8'
...
--WITH temp(BIZ_ORGAN_TYPE,BIZ_TYPE_NAME,parent_type) as (select organ_type as BIZ_ORGAN_TYPE ,type_name as BIZ_TYPE_NAME,parent_type from pub_organ_type where parent_type in ('1','2','6')  and organ_type<>parent_type  UNION ALL (select a.organ_type as BIZ_ORGAN_TYPE,a.type_name as BIZ_TYPE_NAME,a.parent_type from pub_organ_type a,temp b where a.parent_type=b.BIZ_ORGAN_TYPE))  select * from temp;

--创建岗位类型视图
--CREATE VIEW V_POST_TYPE  AS  WITH temp(POST_TYPE,POST_TYPE_NAME,parent_type) as (select organ_type as POST_TYPE ,type_name as POST_TYPE_NAME,parent_type from pub_organ_type where parent_type='6'  and organ_type<>parent_type  UNION ALL (select a.organ_type as POST_TYPE,a.type_name as POST_TYPE_NAME,a.parent_type from pub_organ_type a,temp b where a.parent_type=b.BIZ_ORGAN_TYPE))  select * from temp;
