...
<%
	if("create".equals(request.getParameter("status"))) {
		String moduleName = URLDecoder.decode(request.getParameter("moduleName"),"utf8");
...
<body>
	<form class="form-horizontal" id="functionForm" onsubmit="return false">
	   <div class="form-group">
	      <label class="col-xs-3 col-md-3 text-right" for="functionName"><spring:message code="bsp.function.021" text="功能名称"/><span class="required">*</span></label>
	      <div class="col-xs-9 col-md-9">
	         <input id="functionCode" name="functionCode" type="hidden" value="${object.functionCode}">
	         <input autofocus="autofocus" id="functionName" name="functionName" class="form-control ue-form Validform_input" type="text" datatype="*1-60" nullmsg="<spring:message code="bsp.function.049" text="名称不能为空"/>" value="${object.functionName}">
	         <span class="Validform_checktip Valiform_span"></span>
	      </div>
	   </div>
	   <div class="form-group">
	      <label class="col-xs-3 col-md-3 text-right" for="moduleName"><spring:message code="bsp.function.050" text="所属模块"/></label>
	      <div class="col-xs-9 col-md-9">
	         <input id="moduleName" readonly="readonly" name="moduleName" class="form-control ue-form Validform_input" type="text" value="${object.moduleName}">
...
$(function(){
	if(status == "create") {
		$("#seq").val("${ param.seq }");
		$("#moduleCode").val("${param.moduleCode}");
...
// 保存
function saveVal() {
	var url = context + "/service/bsp/function/updateFunction";
	if(status == "create") {
		url = context + "/service/bsp/function/createFunction";
	}
